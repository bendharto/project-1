...
//double *Ti=(double*)mxGetPi(prhs[2]);
    int FA11,FA21,FA31,FA41,K11,K21,K31,K41;
    //FA12,FA22,FA32,FA42,K12,K22,K32,K42;
    //FA4 = 0;
    FA11 = dim_arrayA1[0];
    FA21 = dim_arrayA1[1];
    FA31 = dim_arrayA1[2];
...
//        // Select a device and display arrayfire info
           ...
           //int total_device = devicecount();
            ...
            //std::cout << "device count = "<< total_device << std::endl;
            //af_info();
            //deviceset(1);
            //af_info();
...
//            std::cout << "d_name= "<< d_name << std::endl;
...
//            array keri;
            ...
            array kerr(K11,K21,K31,Br1);
            array keri(K11,K21,K31,Bi1);
//
//            int ll = floor(FA41/2);
//
//            if(ik==0){
//
//            dat_muxr = dat_muxr(span,span,span,seq(0,ll));
//            dat_muxi = dat_muxi(span,span,span,seq(0,ll));
//            kerr = kerr(span,span,span,seq(0,ll));
//            keri = keri(span,span,span,seq(0,ll));
//            p4 = ll;
//            }
//
//            if(ik==1){
//
//            dat_muxr = dat_muxr(span,span,span,seq(ll,end));
//            dat_muxi = dat_muxi(span,span,span,seq(ll,end));
//            kerr = kerr(span,span,span,seq(ll,end));
//            keri = keri(span,span,span,seq(ll,end));
//            p4 = FA41-ll;
//            }
...
//            row = K4;
            ...
            array A = complex(dat_muxr,dat_muxi).as(c32);
            array B = complex(kerr,keri).as(c32);
...
//            B = B(span,span,span,i+p4a*k);}
            int ndim = 2;
            const size_t dims5[2] = {1,p1*p2*p3*p4};
             plhs[0] = mxCreateNumericArray( ndim,dims5,mxSINGLE_CLASS,mxCOMPLEX);

             h_ptr = (float *)mxGetData(plhs[0]);
             //std::cout << "dim B4 = "<< B.dims(0) << std::endl;
             h_pti = (float *)mxGetImagData(plhs[0]);
...
//            array hp = constant(0,p1,p2*p3*p4,c64);
//            int xto = p2*p3*p4;
//            for(int k=0;k<xto;k = k + 1){
//            if(k%2 == 0){
//            hp(span,k)=A(span,k);
//            }else{
//            hp(span,k)=-1*A(span,k);
//            }
//            }
//
//            A = moddims(hp,p1,p2,p3,p4);
...
//            }
//            }
//            hp(span,seq(0,end,2),span,span) = A(span,seq(0,end,2),span,span);
//            hp(span,seq(1,end,2),span,span) = -1*A(span,seq(1,end,2),span,span);
//
//            A = hp;
            ...
            for(int k=0;k<p4;k = k + 1){
            //gfor(seq k,p4){
            //A(span,seq(1,end,2),span,k) = -1*A(span,seq(1,end,2),span,k);
...
//
//            C = shift(A(span,span,span,k),x1,y1,z1,w1);
...
//            C = ifft(shift(A(span,span,span,k),x1,y1,z1,w1));
//            C = shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1);
//            C = shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1)*B;
//            C = shift(shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1)*B,x1,y1,z1,w1);
//            C = fft(shift(shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1)*B,x1,y1,z1,w1));
//            C = shift(fft(shift(shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1)*B,x1,y1,z1,w1)),-x1,-y1,-z1,-w1);
            C = moddims(shift(fft(shift(shift(ifft(shift(A(span,span,span,k),x1,y1,z1,w1)),-x1,-y1,-z1,-w1)*B,x1,y1,z1,w1)),-x1,-y1,-z1,-w1),1,p1*p2*p3);
