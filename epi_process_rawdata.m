...
%   dat_ecc    - The input dat_ecc which has gone through chopping and ramp sampling correction.
%                This is nonempty only when p.cap_get_ecc == true && p.output_ecc_dat == true.
%
...
% -- Save to or load from the paramter structure
save_load_params = (p.calc_snr_pmr || p.calc_sense_rsnr || p.RT_flag);
if save_load_params
    if p.cap_get_ecc
...
% fprintf('check params : %f seconds\n',toc(ax8));
% ax7 =tic;
% -- Data whitening
...
% fprintf('data whitening : %f seconds\n',toc(ax7));
% ax6 = tic;
...
%         ax52 = tic;
        switch p.ref_for_default_ecc
            case 'ecc data'
                [dat, p] = default_ecc(dat, p, dat_ecc);  % Use reference k-space data collected by the mux sequence in default correction
            case 'ref pfile'
                [dat, p] = default_ecc(dat, p, refp, slices+(floor((p.mux-1)/2))*p.num_slices); % Use reference scan pfile in default correction
            case 'ref.dat'
                [dat, p] = default_ecc(dat, p, ref, slices); % Use ref.dat file in default correction
        end
%         fprintf('Nyquist ghost default_ecc : %f seconds\n',toc(ax52));
    else
%         ax51 = tic;
        if p.md_ecc
...
%         fprintf('Nyquist ghost md_ecc : %f seconds\n',toc(ax51));
    end
...
%             fprintf('ecc data \n');
        case 'ref pfile'
            [dat, p] = default_ecc(dat, p, refp, slices_ref); % Use reference scan pfile in default correction
%             fprintf('ref file \n');
        case 'ref.dat'
            [dat, p] = default_ecc(dat, p, ref, slices); % Use ref.dat file in default correction
%             fprintf('ref dat \n');
    end
%     fprintf('Nyquist default_ecc else : %f seconds\n',toc(ax5x));
end

% fprintf('Nyquist ghost : %f seconds\n',toc(ax5));
% ax4 = tic;
% -- Ramp sampling correction
if p.vrgf
    % Ramp sampling correction
    if save_load_params && isfield(p, 'ramp_flt') && ~isempty(p.ramp_flt)
        ramp_flt = p.ramp_flt;
    else
    ...
    if p.cap_get_ecc && p.output_ecc_data
        dat_ecc = epi_vrgf_correct(dat_ecc, ramp_flt);
    end
    
    % Nyquist ghost correction: matrix decoding correction for ramp on data
    if ~p.RT_flag && p.md_ecc % TODO: currently not working for real-time recon
        [dat, p.pha_coe] = md_ecc(dat, p, md_ref, p.pha_coe_default, ramp_flt, slices_ref);
    end
    ...
    if ismember(p.mux_recon_method, {'slice-grappa', 'split-slice-grappa'}) && p.slice_grappa_odd_even_fit
        [~, p.pha_coe] = md_ecc([], p, md_ref, p.pha_coe_default, ramp_flt, slices_ref);
    end
    
    if save_load_params && ~isfield(p, 'ramp_flt')
        p.ramp_flt = ramp_flt;
    end
else
    if save_load_params && ~isfield(p, 'ramp_flt')
...
% fprintf('ramp sampling : %f seconds\n',toc(ax4));

% -- Fermi filtering
% ax2 = tic;
if p.apply_fermi
    if save_load_params && isfield(p, 'fermi_filt') && ~isempty(p.fermi_filt)
        fermi_filt = p.fermi_filt;
    else
    ...
    if p.use_GPU
        dat = bsxfun(@times, dat, fermi_filt);
    else
    ...
    if save_load_params && ~isfield(p, 'fermi_filt')
        p.fermi_filt = fermi_filt;
    end
end
% fprintf('fermi : %f seconds\n',toc(ax2));
...
% -- Whether to output the internal eddy current correction data
if ~(p.cap_get_ecc && p.output_ecc_data)
    dat_ecc = [];
end
% fprintf('Notch : %f seconds\n',toc(ax3));
...
function [dat, p] = default_ecc(dat, p, ref, slices)
%
