function [dat, p, dat_ecc] = epi_load_tseries(pfile, ref, vrgf, refp, slices, slices_pfile, slices_ref, p,datatot,position,re)
% dbstackk = dbstack;
% disp([dbstackk(2).file mfilename('fullpath')])
...
% -- Load k-space data
[dat, p] = load_raw_tseries(pfile, p, slices_pfile, p.tpoints_to_load, datatot,position,re); % Dim: [FE, PE, Echo, Slice, Coil, Time]

aq1 = tic;

% -- Reference data for Nyquist ghost correction
datsz = get_dat_sz(dat, p);
if p.cap_get_ecc && p.cal_flag
    ...
    p.dat_ecc = reshape(dat_ecc, [datsz.x, datsz.y, datsz.ec, datsz.sl*p.mux_excited, datsz.c]); % Dim: [Kx, Ky, Echo, Slice->SimultaneousSliceZ(num_muxed_slices*mux_excited), Coil]
    
    dat = dat(:, :, :, :, :, p.mux_excited+1:end);
else
    p.dat_ecc = [];
end
fprintf('after rawloadX Nyquist : %f seconds\n',toc(aq1));
aq11 = tic;
% -- Deal with the case where extra slices are encoded in calibration time points but won't be used in recon
if (p.mux_encoded > p.mux_excited) && ~p.recon_cal_use_mux_extra && p.cal_flag
...
fprintf('after rawloadX deal with slices : %f seconds\n',toc(aq11));
aq12 = tic;
% -- Save the raw data for pseudo-multiple replica simulation
if p.RT_flag && p.cal_flag && ((p.calc_snr_pmr || p.calc_sense_rsnr) || ...
        (strcmp(p.mux_recon_method, 'slice-grappa') || strcmp(p.mux_recon_method, 'split-slice-grappa')))  % slice-grappa recon needs raw data to synthesize muxed data point for kernel fitting
    p.pmr_raw_data = dat(:, :, :, :, :, p.mux_encoded * (p.num_mux_cycle-1) + 1 : p.mux_encoded * p.num_mux_cycle);
end
fprintf('after rawloadX save data for fitting : %f seconds\n',toc(aq12));
aq13 = tic;
% -- Process the raw data
[dat, p, dat_ecc] = epi_process_rawdata(dat, p, p.dat_ecc, ref, vrgf, refp, slices, slices_ref);
fprintf('Process the raw data : %f seconds\n',toc(aq13));
fprintf('after rawloadX : %f seconds\n',toc(aq1));
