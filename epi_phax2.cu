...
//                Bimag.host(h_pti);
            ...
            //gfor(seq k,p4){
            //for(int kc=0;kc<p3;kc = kc + 1){
            //A(span,seq(1,end,2),kc,k) = -1*A(span,seq(1,end,2),kc,k);}
           ...
           // A = shift(A(span,span,span,i+p4a*k),x1,y1,z1,w1);}
            ...
            C = (sqrt(C.dims(0)*C.dims(1)))*ifft(C);
            C = shift(C,-x1,-y1,-z1,-w1);
            C = C*B;
            C = shift(C,x1,y1,z1,w1);
            C = (1/sqrt(C.dims(0)*C.dims(1)))*fft(C);
            C = shift(C,-x1,-y1,-z1,-w1);
            C = moddims(C,p1*p2*p3,1);
            if(k==0){
            D = C;
            }else{
            ...
            if(k % pan == 0){
             ...
             Bimag.host(h_pti);
             D = C;
             h_ptr = h_ptra + ((k)*(p1*p2*p3));
             h_pti = h_ptia + ((k)*(p1*p2*p3));
